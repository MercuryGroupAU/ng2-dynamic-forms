export class Utils {
	...
	static addDaysToToday(days: number):string {
		...
		let dateString = curr_year + "-" + this.doubleDigit(curr_month) + "-" + this.doubleDigit(curr_date);
		return dateString;
	}
	
	//input type date needs 2 digit numbers for day and month
	static doubleDigit(n:number):string {
		return n > 9 ? "" + n: "0" + n;
	}
		
	static datePickerAddDaysToToday(days: number):any {
		let date: Date;
		date = new Date();
		date.setDate(date.getDate() + days);
